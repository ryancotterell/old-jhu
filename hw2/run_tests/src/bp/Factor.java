...
//				psi.table[i] = this.table[j] / other.table[k];
	...
	{
		// if the list is empty, we don't marginalize anything. Copy this
		if (variables.size() == 0)
		{
			Factor factor = new Factor(this.variables);
			double[] newTable = new double[this.table.length];

			for (int i = 0; i < this.table.length; i++)
				newTable[i] = this.table[i];
		
			factor.setTable(newTable);
			return factor;
		}
