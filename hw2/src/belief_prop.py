...
class Factor:
  ...
  def __init__(self, rvs):
    self.rvs = rvs.sort()
    self.cpt = _init_cpt(rvs)
    
  def _init_cpt(self, rvs):
    dims_list = []
    for r in rvs:
      dims_list.append(len(r.vals))
  ...
  def update_cpt_entry(entry, value):
    ...
    for rv_name, rv in zip(sorted(entry), self.rvs):
      ...
      indices.add(rv.get_index(entry[rv_name])
    self.cpt[tuple(indices)]
  def __str__(self):
...
class Node:
  ...
  def __str__(self):
    return ('name: ' + self.name + 'values: ' + str(self.values) + '\n')
...
def parse_cpd(nodes, factors, cpd_file): 
  ...
  for line in cpd:
    ...
    variables = variables + halves[2].split(',')
    entry = {}
